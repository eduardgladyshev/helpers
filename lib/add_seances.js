...
function checkSales(seances){
  ...
  if(seancesWithSales.length) {
    let ids = seancesWithSales.map(i => i.id);
    console.info(ids);
    throw new Error('Has open sale seances');
  }
}
...
async function createSchedule(options){
  ...
  try{
    ...
    let ids = addRes.body.map(i => i.id);
